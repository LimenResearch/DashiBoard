...
function applyFilter(obj, val) {
  return (
    obj && _.entries(obj).every(([key, options]) => options.includes(val[key]))
  );
}
...
export function cardValue(config) {
  ...
  const validated = config.fields.every(
    (x) => !isRequired(x, g) || isValid(x, g),
  );
...
export function Card(props) {
  ...
  const names = () =>
    metadata.concat(otherCards().flatMap(getOutputs)).map((x) => x.name);
...
export function CardPlus(props) {
  ...
  const plusClass =
    "h-7 font-bold text-lg text-gray-900 hover:text-gray-1000 hover:bg-gray-200";

  return (
    <div class="grid grid-cols-2 gap-4 py-2 my-2">
