...
function formatter(val: any, eltype: string) {
  if (val == null) {
    return null;
  }
  switch (eltype) {
...
export function TableView(props: TableViewProps) {
  const dataSource = (columnDefs: {length: number}) => {
    if (columnDefs.length == 0) {
      return {
        rowCount: 0,
        getRows: (params: any) => params.successCallback([], 0),
      };
    } else {
      return {
  ...
  const options = () => {
    const columnDefs = props.metadata.map((x: {name: string, eltype: string}) => ({
      field: x.name,
      headerName: x.name,
      valueFormatter: (params: {value: any}) => formatter(params.value, x.eltype),
    }));
  ...
  let gridApi: GridApi, gridDiv!: HTMLDivElement;

  onSettled(() => {
    gridApi = createGrid(gridDiv, gridOptions);
  });

  createEffect(options, _options => gridApi.updateGridOptions(_options));

  // TODO: avoid hard-coded height?
  return (
    <div ref={gridDiv} style="height: 500px" class="ag-theme-quartz"></div>
  );
}
