...
| A2 | ~~Delete `assets/config/*.toml`, every `CardWidget` method, `card_widgets()`, and `widgets.jl`~~ **DONE 2026-09-13** | Verified: nothing references `Widget` or `OutputSpec` anywhere. ~330 lines of Pipelines plus 11 asset files, the `/get-card-widgets` route and its handler, `Pipelines/test/frontend.jl`, and `frontend/` (34 tracked files). Two dependencies went with it — `TOML` and `RelocatableFolders` existed in `Pipelines/src` *only* for `parse_toml_config`. **Done ahead of A1, deliberately and at a cost:** the 21 `label`/`placeholder` strings in those TOMLs were the only copy anyone had written, so they are recorded in issue #167 before deletion rather than left to git archaeology. Two things the capture showed: `label` is A3b's `title`, not `description`, and no field ever had a sentence explaining what it *does* — so A1 is mostly authoring, not migration. |
...
| A11 | ~~Collect schema failures rather than throwing on the first~~ **DONE 2026-09-14** — `SchemaValidationErrors` carries them all, groups and cards alike, and `issue_report` has a method for it | Also **`Pipelines.card_issues`**, the targeted counterpart: one card, its own schema, no graph walk — served as `POST /validate-card`. Together these retired the UI's `checkFields`, which was measured to find exactly the set the card's schema already reports. What stays in the UI is `checkGroup` and `checkNode`, the two documents the server accepts. Note the remaining limit, which is JSONSchema.jl's: `validate` returns a `SingleIssue`, so this widened the report *across* a document, not within a card. |
| A12 | `json_response` cannot encode a non-finite number | **Measured 2026-09-13** by smoke test. A zscore of a zero-variance column produces `NaN`, which reaches `summaries`, and `JSON.json` then throws `ArgumentError: NaN not allowed to be written in JSON spec`. The pipeline *succeeded*; the response could not be written — so a good run is reported to the client as an execution failure, and before the error work it was a blank screen. `allownan = true` is not the fix: `NaN` is not JSON and the browser's parser rejects it. Map non-finite floats to `null`, which is the convention every JSON client already understands. Any constant column meets this, which is not exotic — an all-one-value column is a normal thing to find in a real table. |
| A13 | A card's output silently replaces an existing column of the same name | **Measured 2026-09-13.** `rescale TEMP suffix = "rescaled"` emits `TEMP_rescaled`; a source that already has a `TEMP_rescaled` is overwritten, and nothing objects at any layer — the UI check, the probe and the run all accept it. This is the confusion that produced the `{cols = ["PRES_rescaled", "TEMP_rescaled"]}` group earlier in this work. The names are computable before running (`output_vars` per card, against `cols`), so the probe is where it belongs — as a warning rather than a rejection, since overwriting may occasionally be meant. |
